config router bgp
    ...
    config neighbor
        edit "169.254.40.9"
            ...
            set description "AWS"
            set interface "VPN-AWS-1" <---
            set maximum-prefix 100
            set remote-as 64513
            set local-as 64512
            set retain-stale-time 9
            set route-map-in "AWS-To-Fortigate-in-Secondary"
            set route-map-out "Fortigate_To_Partners_Out_Secondary"
            set keep-alive-timer 10
            set holdtime-timer 30
            set connect-timer 10
            set update-source "VPN-AWS-1" <---
            set restart-time 10
...
config router route-map
    edit "AWS-To-Fortigate-in-Secondary"
        config rule
            edit 1
                set match-ip-address "AWS-Prefix"
                set set-local-preference 900
                set set-metric 100
                set set-weight 900
            next
            edit 2
